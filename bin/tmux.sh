...
if [ "$1" = "-h" ] || [ "$1" == "--help" ]; then
	printf "\n"
	printf "\033[1m  t - tmux session manager\033[0m\n"
	printf "\n"
...
if [ "$SESSION" = "" ]; then # session is missing
	...
	if [ -e $RESULT/.t ]; then
		#tmux new-session -d -s "$SESSION" "top" -c "$RESULT" $RESULT/.t \; # create session and run .t startup script
		tmux new-session -d -s "$SESSION" "top" -c "$RESULT" "nvim ."
