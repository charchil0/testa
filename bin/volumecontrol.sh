...
notify_mute() {
    ...
    if [ "${mute}" == "true" ]; then
        notify-send -a "t2" -r 91190 -t 1000 -i "${icodir}/muted-${dvce}.svg" "muted" "${nsink}"
    else
        notify-send -a "t2" -r 91190 -t 1000 -i "${icodir}/unmuted-${dvce}.svg" "unmuted" "${nsink}"
    fi
}
...
select_output() {
    ...
    if [ -n "$selection" ]; then
        ...
        if pactl set-default-sink "$device"; then
            notify-send -t 2000 -r 3 -u low "Activated: $selection"
        else
            notify-send -t 2000 -r 3 -u critical "Error activating $selection"
        fi
    else
